...
class _TopSellerState extends State<TopSeller> {
  ...
  getProducts() async {
    ...
    var res = await wc_api
        .getAsync("products?status=publish&per_page=20");
    ...
    if (res.statusCode == 200 && decodedJson != null) {
      if (mounted) {
        setState(() {
          ...
          original_list.sort((a, b) => a["total_sales"].compareTo(b["total_sales"]));
          productsList = original_list.reversed.toList();
    ...
    return Scaffold(
      ...
      actions: <Widget>[
          IconButton(
            ...
            onPressed: () async {
                Navigator.push(context, MaterialPageRoute(builder: (context)=>Search(id: 0,name: "")));
